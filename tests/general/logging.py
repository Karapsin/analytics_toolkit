...
def test_time_print_can_emit_through_logging(
    ...
    assert [(record.name, record.levelname, record.getMessage()) for record in caplog.records] == [
        (
            "analytics_toolkit",
            "INFO",
...
def test_time_print_context_includes_task_id(
    ...
    assert capsys.readouterr().out == "[2026-06-03 18:00:00] [task_id=orchestrator] running\n"
