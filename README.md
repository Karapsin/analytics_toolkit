...
with optional CUPED statistics, bootstrap multiple-comparison adjustment, and
total plus per-value segment slices.
...
Pass `segment="country_code"` to prepend a `segment` column containing a
`TOTAL` block followed by each observed country-code value.
